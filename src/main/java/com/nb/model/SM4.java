...
public class SM4 {
	...
	public static String[] parentKey = { "oaa6FQECDAxGq62P", "8V3F0oyKGROjjLP7", "x3EMuvv69ZzV8TP9", "jVL0iGwO85e8Nl5A",
			"lf7ezOg261vq2kpU", "Na1Ri7X7Ikw0lWXw", "5zyb3k8pp7jvqXAx", "SYP8UgTKuZVrwemS", "qAiaOADmu3B7EXB5",
			"PzgI5qHJCqC0gVve" };
	...
	* @throws 
	*/
	public static String getSecretKey(int keyNo, String imei) {

		byte[] pKey = parentKey[keyNo].getBytes();
		byte[] imeiBytes = String.format("%016d", ConverterUtils.toLong(imei)).getBytes();
		for (int i = 0; i < imeiBytes.length; i++) {
			imeiBytes[i] = (byte) (imeiBytes[i] ^ pKey[i]);
		}
		return new String(imeiBytes);
	}
